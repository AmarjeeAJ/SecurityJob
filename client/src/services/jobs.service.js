import ROLE_SLUGS from '../utils/roleSlugs.js';
import { INDIAN_CITIES } from '../utils/locations.js';

// Comprehensive, realistic security industry job catalog matching all 19 backend roles
export const JOBS_CATALOG = [
  {
    id: 'sj-001',
    slug: 'security-guard',
    title: 'Security Guard',
    category: 'Guard & Protection',
    popular: true,
    featured: true,
    employer: 'Premier Facility & Security Services',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Udaipur', 'Jodhpur', 'Kota', 'Pune', 'Bengaluru'],
    primaryLocation: 'Jaipur & Delhi NCR',
    salaryMin: 16000,
    salaryMax: 24000,
    salaryPeriod: 'month',
    salaryDisplay: '₹16,000 – ₹24,000 / month',
    experienceRequired: '0 - 3 years (Freshers Welcome)',
    experienceLevel: 'Fresher / Experienced',
    qualification: '10th Pass / 12th Pass',
    shift: '8 Hours / 12 Hours (Day/Night Rotational)',
    jobType: 'Full-time',
    vacancies: 45,
    genderEligibility: 'Male / Female',
    postedDate: 'Recently Posted',
    summary: 'Responsible for safeguarding client premises, access control, monitoring visitor logs, and maintaining perimeter security.',
    overview: 'Join leading security agency deployments across commercial complexes, IT parks, retail malls, residential societies, and industrial premises. Free uniforms and standard statutory benefits provided by employers.',
    responsibilities: [
      'Perform access control and identity verification at entry/exit gates',
      'Maintain visitor, vehicle, and material inward/outward registers',
      'Conduct regular foot patrols across designated facility perimeters',
      'Monitor safety hazards and report incidents promptly to supervisors',
      'Assist visitors with basic directions while upholding site decorum',
      'Operate hand-held metal detectors (HHMD) and under-vehicle search mirrors when required',
    ],
    requirements: [
      'Minimum age: 18 years, Maximum age: 50 years',
      'Physical fitness and clean background verification',
      'Ability to read and write basic Hindi/English for register entries',
      'Aadhaar card and local address proof',
    ],
    benefits: [
      'PF (Provident Fund) and ESIC health coverage',
      'Uniform & duty shoes provided',
      'Overtime allowance as per site norms',
      'On-site accommodation / hostel assistance at select locations',
    ],
    documentsRequired: [
      'Aadhaar Card (Front & Back)',
      '10th / School Leaving Certificate or Marksheet',
      '2 Passport Size Photographs',
      'Bank Account Passbook / Cancelled Cheque',
    ],
  },
  {
    id: 'sj-002',
    slug: 'lady-security-guard',
    title: 'Lady Security Guard',
    category: 'Guard & Protection',
    popular: true,
    featured: true,
    employer: 'Apex Facility Solutions & Hospital Security',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Pune', 'Bengaluru'],
    primaryLocation: 'Delhi NCR, Jaipur & Mumbai',
    salaryMin: 17000,
    salaryMax: 25000,
    salaryPeriod: 'month',
    salaryDisplay: '₹17,000 – ₹25,000 / month',
    experienceRequired: '0 - 2 years (Freshers Welcome)',
    experienceLevel: 'Fresher / Experienced',
    qualification: '10th Pass / 12th Pass',
    shift: '8 Hours / 12 Hours (Day Shifts Preferred)',
    jobType: 'Full-time',
    vacancies: 30,
    genderEligibility: 'Female Only',
    postedDate: 'Recently Posted',
    summary: 'Dedicated female security personnel for frisking, access control, and visitor assistance at hospitals, corporate offices, schools, and malls.',
    overview: 'Specialized female security roles in safe, structured environments including international schools, female staff corporate campuses, shopping malls, and healthcare institutions. Safe commuting options and day-shift preferences available.',
    responsibilities: [
      'Conduct respectful frisking and bag checks for female visitors and employees',
      'Monitor female-dedicated floors, locker rooms, and restricted office zones',
      'Assist female visitors, students, and patients with courteous orientation',
      'Maintain front desk and reception security entry registers',
      'Coordinate with senior security control room officers on duty alerts',
    ],
    requirements: [
      'Age: 18 to 45 years',
      'Height: 152 cm (5 ft) or above preferred',
      'Pleasant demeanor, punctual, and disciplined',
      'Basic communication skills in Hindi and regional language',
    ],
    benefits: [
      'PF, ESIC and statutory medical benefits',
      'Day shifts with scheduled weekly offs',
      'Safe corporate and educational campus work environments',
      'Uniform provided by agency',
    ],
    documentsRequired: [
      'Aadhaar Card',
      'Educational Qualification Marksheet',
      'Bank Account Details',
      'Photographs',
    ],
  },
  {
    id: 'sj-003',
    slug: 'security-supervisor',
    title: 'Security Supervisor',
    category: 'Supervision & Management',
    popular: true,
    featured: true,
    employer: 'Frontline Security Management Services',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Ahmedabad', 'Pune', 'Udaipur'],
    primaryLocation: 'Jaipur, Gurugram & Noida',
    salaryMin: 24000,
    salaryMax: 36000,
    salaryPeriod: 'month',
    salaryDisplay: '₹24,000 – ₹36,000 / month',
    experienceRequired: '2 - 6 years in Security / Facility',
    experienceLevel: 'Experienced (2+ Years)',
    qualification: '12th Pass / Graduate',
    shift: '12 Hours (Rotational)',
    jobType: 'Full-time',
    vacancies: 20,
    genderEligibility: 'Male / Female',
    postedDate: '2 days ago',
    summary: 'Lead guard shifts, conduct daily briefings, manage site attendance rosters, and coordinate emergency responses.',
    overview: 'Supervisory role overseeing on-site guard deployments, shift handover protocols, client escalation management, and site safety audits across corporate headquarters, manufacturing plants, and luxury residential complexes.',
    responsibilities: [
      'Conduct daily guard muster roll, turnout inspection, and post allocations',
      'Oversee visitor management, material passes, and loading bay protocols',
      'Act as primary point of contact for site client managers and administrative officers',
      'Investigate site incidents, draft initial incident reports, and preserve evidence',
      'Conduct bi-weekly mock drills for fire safety and emergency evacuation',
      'Ensure 100% adherence to PSARA guidelines and client SOPs',
    ],
    requirements: [
      'Minimum 2 years experience as a Senior Guard or Security Supervisor',
      'Working knowledge of security registers, CCTV overview, and basic smartphone reporting apps',
      'Strong leadership, team coordination, and crisis de-escalation skills',
      'Ex-servicemen / NCC certificate holders given priority',
    ],
    benefits: [
      'Attractive performance incentive and supervisory allowance',
      'PF, ESIC, and annual bonus as per labour standards',
      'Clear career progression to Assistant Field Officer (AFO) and Field Officer',
    ],
    documentsRequired: [
      'Aadhaar Card & PAN Card',
      'Experience Certificates / Relieving Letters from past security agencies',
      '12th / Degree Certificate',
      'Bank Passbook & Photos',
    ],
  },
  {
    id: 'sj-004',
    slug: 'cctv-operator',
    title: 'CCTV Operator',
    category: 'Technical & Control Room',
    popular: true,
    featured: true,
    employer: 'Integrated Surveillance & Facility Command',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Bengaluru', 'Hyderabad'],
    primaryLocation: 'Delhi NCR & Jaipur',
    salaryMin: 20000,
    salaryMax: 30000,
    salaryPeriod: 'month',
    salaryDisplay: '₹20,000 – ₹30,000 / month',
    experienceRequired: '1 - 4 years in Control Room Operations',
    experienceLevel: 'Experienced',
    qualification: '12th Pass / ITI / Diploma / Graduate',
    shift: '8 Hours / 12 Hours (Rotational)',
    jobType: 'Full-time',
    vacancies: 18,
    genderEligibility: 'Male / Female',
    postedDate: 'Recently Posted',
    summary: 'Monitor live multi-channel surveillance feeds, detect perimeter intrusions, log security alarms, and coordinate field teams.',
    overview: 'Control room surveillance specialist responsible for active video wall monitoring, recording audits, alarm acknowledgement, and dispatching on-ground quick response teams across high-security corporate sites and retail malls.',
    responsibilities: [
      'Actively monitor 32+ camera feeds on VMS (Video Management Software) consoles',
      'Identify unauthorized access, suspicious loitering, unattended baggage, and fire alarms',
      'Dispatch patrolling guards via two-way wireless radio systems (walkie-talkie)',
      'Maintain CCTV daily incident logbooks and archive footage requested for investigations',
      'Check system health: camera uptime, NVR/DVR recording status, and storage retention',
    ],
    requirements: [
      'Experience operating Hikvision, Dahua, Honeywell, Milestone, or Bosch VMS',
      'Basic computer proficiency (MS Excel, Email reporting, Video playback)',
      'Sharp attention to detail and ability to remain alert during night shifts',
    ],
    benefits: [
      'Air-conditioned command center work environment',
      'PF, ESIC, Night Shift Allowance',
      'Career growth into Central Control Room (CCR) Manager roles',
    ],
    documentsRequired: [
      'Aadhaar Card',
      'Computer / CCTV Training Certificate (if available)',
      'Educational Certificates',
      'Experience Proof',
    ],
  },
  {
    id: 'sj-005',
    slug: 'armed-guard',
    title: 'Armed Guard',
    category: 'Guard & Protection',
    popular: true,
    featured: true,
    employer: 'SecureCash & High-Value Asset Protection',
    locations: ['Jaipur', 'Delhi', 'Jodhpur', 'Kota', 'Udaipur', 'Lucknow', 'Mumbai'],
    primaryLocation: 'Rajasthan & North India',
    salaryMin: 26000,
    salaryMax: 40000,
    salaryPeriod: 'month',
    salaryDisplay: '₹26,000 – ₹40,000 / month',
    experienceRequired: '2+ years with valid gun licence',
    experienceLevel: 'Licensed Gunman / Armed Specialist',
    qualification: '10th Pass / Ex-Servicemen Preferred',
    shift: '8 Hours / 12 Hours (Day/Night)',
    jobType: 'Full-time',
    vacancies: 15,
    genderEligibility: 'Male',
    postedDate: '3 days ago',
    summary: 'Licensed armed personnel for banking institutions, jewellery vaults, cash management, and high-value cargo protection.',
    overview: 'High-responsibility armed security roles offering competitive compensation for candidates with valid Indian Arms licences (DBBL / SBBL / Revolver / Pistol). Weapons maintenance and safe handling mandatory.',
    responsibilities: [
      'Provide lethal and non-lethal armed deterrent at banking branches, ATM vaults, and currency chests',
      'Escort cash-in-transit transit vehicles during intra-city routes',
      'Safeguard VIP clients and high-value commercial assets',
      'Maintain arms cleaning logs and ensure ammunition safety compliance',
    ],
    requirements: [
      'Valid Indian Arms Licence with All-India or State jurisdiction',
      'Ex-Servicemen (Army, Navy, Air Force, BSF, CRPF, CISF, Police) preferred',
      'Sound mental and physical fitness certificate',
    ],
    benefits: [
      'High monthly remuneration with weapon allowance',
      'Insurance coverage and medical benefits',
      'Prompt monthly salary disbursement',
    ],
    documentsRequired: [
      'Valid Indian Gun Licence Book (All pages)',
      'Discharge Book / Service Record (for Ex-Servicemen)',
      'Aadhaar & PAN Card',
      'Weapon Registration Proof',
    ],
  },
  {
    id: 'sj-006',
    slug: 'bouncer',
    title: 'Bouncer',
    category: 'Guard & Protection',
    popular: true,
    featured: true,
    employer: 'Elite Event Security & Hospitality Group',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Pune', 'Goa', 'Chandigarh'],
    primaryLocation: 'Delhi NCR, Jaipur & Mumbai',
    salaryMin: 22000,
    salaryMax: 35000,
    salaryPeriod: 'month',
    salaryDisplay: '₹22,000 – ₹35,000 / month',
    experienceRequired: '1 - 5 years in Crowd Control / Nightclub Security',
    experienceLevel: 'Experienced (Physique & Height Criteria)',
    qualification: '10th Pass / 12th Pass',
    shift: 'Evening / Night Shifts & Weekend Events',
    jobType: 'Full-time / Event Contract',
    vacancies: 25,
    genderEligibility: 'Male / Female',
    postedDate: 'Recently Posted',
    summary: 'Professional crowd control, VIP protection, and venue safety at high-end clubs, private events, weddings, and concerts.',
    overview: 'Immediate requirements for disciplined, well-built bouncers for luxury hospitality venues, high-profile celebrity events, exhibitions, and upscale lounges. Professional conduct and de-escalation skills mandatory.',
    responsibilities: [
      'Manage venue entrance queues, age verification, and dress code checks',
      'Prevent entry of unauthorized persons, weapons, and banned substances',
      'Diffuse verbal altercations and escort disruptive guests off premises safely',
      'Maintain close security around VIP tables, stages, and private banquet halls',
      'Coordinate with local law enforcement and venue managers during emergencies',
    ],
    requirements: [
      'Height: 5ft 10in (178 cm) or above for male; 5ft 6in (168 cm) for female',
      'Strong athletic physique and clean police background record',
      'Calm temperament under pressure with strong communication skills',
    ],
    benefits: [
      'Competitive pay with event-based tip sharing and food provided on shift',
      'Flexible options for full-time club contracts or high-pay weekend assignments',
      'Branded dark suit / tactical uniform provided',
    ],
    documentsRequired: [
      'Aadhaar Card',
      'Full-Length Photograph showing physique',
      'Police Clearance / Verification Certificate',
      'Bank Account Details',
    ],
  },
  {
    id: 'sj-007',
    slug: 'field-officer',
    title: 'Field Officer',
    category: 'Supervision & Management',
    popular: false,
    featured: false,
    employer: 'National Security Agency Operations',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Ahmedabad', 'Mumbai'],
    primaryLocation: 'Jaipur & Delhi NCR',
    salaryMin: 28000,
    salaryMax: 42000,
    salaryPeriod: 'month',
    salaryDisplay: '₹28,000 – ₹42,000 / month',
    experienceRequired: '3 - 8 years in Security Operations',
    experienceLevel: 'Senior Operations',
    qualification: 'Graduate / Ex-Defence JCO / Police',
    shift: 'General Shift + Night Surprise Audits',
    jobType: 'Full-time',
    vacancies: 10,
    genderEligibility: 'Male',
    postedDate: '4 days ago',
    summary: 'Multi-site operational management, client relationship handling, guard deployment audits, and recruitment coordination.',
    overview: 'Lead field operations across 10 to 20 client sites. Ensure 100% guard turnout, resolve site billing hours, conduct client feedback meetings, and manage emergency replacements.',
    responsibilities: [
      'Conduct scheduled day visits and surprise night inspections across assigned units',
      'Monitor guard attendance, uniform compliance, weapon handling, and register records',
      'Meet site facility managers weekly to review security performance and solve escalations',
      'Manage guard shortfalls by deploying reliever staff promptly',
      'Coordinate with branch office for payroll signoffs, OT validation, and new guard induction',
    ],
    requirements: [
      'Two-wheeler with valid driving licence mandatory',
      'Prior experience managing 100+ security personnel in agency operations',
      'Good written & spoken communication skills in Hindi & English',
    ],
    benefits: [
      'Fuel allowance and travel reimbursement',
      'Mobile bill reimbursement',
      'PF, ESIC, and annual performance incentive',
    ],
    documentsRequired: [
      'Aadhaar Card, PAN Card, Driving Licence',
      'Past Employer Relieving Letters',
      'Degree Certificates',
    ],
  },
  {
    id: 'sj-008',
    slug: 'bodyguard',
    title: 'Bodyguard / PSO',
    category: 'Guard & Protection',
    popular: false,
    featured: false,
    employer: 'Executive Close Protection Services',
    locations: ['Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Jaipur', 'Chandigarh'],
    primaryLocation: 'Delhi NCR & Mumbai',
    salaryMin: 35000,
    salaryMax: 60000,
    salaryPeriod: 'month',
    salaryDisplay: '₹35,000 – ₹60,000 / month',
    experienceRequired: '3+ years in VIP Protection / Commando Background',
    experienceLevel: 'Executive Protection Specialist',
    qualification: '12th Pass / Graduate / Ex-Army / Para / NSG',
    shift: 'Flexible / As per Principal Schedule',
    jobType: 'Full-time',
    vacancies: 8,
    genderEligibility: 'Male / Female',
    postedDate: '5 days ago',
    summary: 'Personal Security Officer (PSO) providing close protection for business leaders, high-net-worth families, and public figures.',
    overview: 'Elite personal security role requiring tactical awareness, route reconnaissance, defensive driving coordination, and unobtrusive escort for principals.',
    responsibilities: [
      'Maintain close personal escort during daily transit, public appearances, and office visits',
      'Perform advance route checks, venue safety inspections, and secure entrance assessments',
      'Manage crowd proximity and prevent unauthorized approaches',
      'Coordinate with principal’s executive chauffeur and administrative team',
    ],
    requirements: [
      'Certified close protection training or ex-special forces / NSG / Commando background',
      'Valid arms licence (Revolver/Pistol preferred for armed PSO roles)',
      'Sharp attire, polite protocol etiquette, and absolute confidentiality',
    ],
    benefits: [
      'Premium compensation package',
      'Travel and accommodation covered during outstation trips',
      'Comprehensive accident insurance',
    ],
    documentsRequired: [
      'Aadhaar & PAN Card',
      'Arms Licence (if applying for Armed PSO)',
      'Military / Commando Service Record (if Ex-Serviceman)',
    ],
  },
  {
    id: 'sj-009',
    slug: 'gunman',
    title: 'Gunman',
    category: 'Guard & Protection',
    popular: false,
    featured: false,
    employer: 'Securitas & Vault Logistics',
    locations: ['Jaipur', 'Delhi', 'Jodhpur', 'Bikaner', 'Ajmer', 'Udaipur'],
    primaryLocation: 'Rajasthan & Delhi NCR',
    salaryMin: 25000,
    salaryMax: 38000,
    salaryPeriod: 'month',
    salaryDisplay: '₹25,000 – ₹38,000 / month',
    experienceRequired: '1+ years with licensed weapon',
    experienceLevel: 'Licensed Gunman',
    qualification: '10th Pass',
    shift: '12 Hours (Day/Night)',
    jobType: 'Full-time',
    vacancies: 20,
    genderEligibility: 'Male',
    postedDate: '1 week ago',
    summary: 'Stationary and mobile armed security for retail jewelers, cash depots, and private industrial estates.',
    overview: 'Deploy with valid 12 Bore DBBL/SBBL or NPB weapon at designated high-value retail and banking sites. Prompt salary with arms maintenance allowance.',
    responsibilities: [
      'Stationed at entry portals of jewellery stores, commercial banks, and gold loan branches',
      'Maintain continuous vigil and challenge unauthorized access attempts',
      'Ensure weapon is handled with safety lock on at all times',
    ],
    requirements: [
      'Valid Gun Licence valid in Rajasthan / All India',
      'Weapon in certified working condition with ammunition permit',
    ],
    benefits: ['PF, ESIC, Monthly Weapon Allowance', 'Permanent site placement'],
    documentsRequired: ['Gun Licence Copy', 'Aadhaar Card', 'Address Proof'],
  },
  {
    id: 'sj-010',
    slug: 'control-room-operator',
    title: 'Control Room Operator',
    category: 'Technical & Control Room',
    popular: false,
    featured: false,
    employer: 'Command Security & BMS Operations',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Pune'],
    primaryLocation: 'Gurugram & Jaipur',
    salaryMin: 22000,
    salaryMax: 32000,
    salaryPeriod: 'month',
    salaryDisplay: '₹22,000 – ₹32,000 / month',
    experienceRequired: '1 - 3 years',
    experienceLevel: 'Experienced',
    qualification: '12th Pass / Diploma / Graduate',
    shift: '8 Hours / 12 Hours (Rotational)',
    jobType: 'Full-time',
    vacancies: 12,
    genderEligibility: 'Male / Female',
    postedDate: '3 days ago',
    summary: 'Coordinate building management systems (BMS), fire alarms, access cards, and incident dispatch.',
    overview: 'Monitor integrated building management and security control consoles in tech parks and large corporate hubs.',
    responsibilities: [
      'Operate access control badge printing and door unlock systems',
      'Monitor fire alarm panels (FAS) and PA (Public Address) systems',
      'Log visitor gate passes and generate weekly surveillance reports',
    ],
    requirements: ['Computer literacy (Excel, Email)', 'Good communication skills'],
    benefits: ['PF, ESIC, AC workplace', 'Career progression to Shift Lead'],
    documentsRequired: ['Aadhaar Card', 'Educational Certificate'],
  },
  {
    id: 'sj-011',
    slug: 'facility-supervisor',
    title: 'Facility Supervisor',
    category: 'Supervision & Management',
    popular: false,
    featured: false,
    employer: 'Integrated Soft & Hard Facility Services',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Ahmedabad'],
    primaryLocation: 'Jaipur & Delhi NCR',
    salaryMin: 25000,
    salaryMax: 38000,
    salaryPeriod: 'month',
    salaryDisplay: '₹25,000 – ₹38,000 / month',
    experienceRequired: '2 - 5 years in Facility Management',
    experienceLevel: 'Experienced',
    qualification: 'Graduate / Diploma',
    shift: 'General Shift (9 AM - 6 PM)',
    jobType: 'Full-time',
    vacancies: 14,
    genderEligibility: 'Male / Female',
    postedDate: 'Recently Posted',
    summary: 'Oversee site housekeeping, security coordination, vendor management, and routine building maintenance.',
    overview: 'Manage combined facility services including housekeeping crews, security personnel, maintenance vendors, and client property upkeep.',
    responsibilities: [
      'Supervise housekeeping staff and inspect cleanliness checklists',
      'Coordinate with electrical and plumbing technicians for prompt repairs',
      'Manage consumable inventory (cleaning chemicals, hygiene items)',
      'Conduct daily walkthroughs with client administration staff',
    ],
    requirements: ['Experience in facility or hospitality operations', 'Customer-service orientation'],
    benefits: ['PF, ESIC, Monthly Bonus, Mobile Allowance'],
    documentsRequired: ['Aadhaar & PAN Card', 'Experience Letters'],
  },
  {
    id: 'sj-012',
    slug: 'housekeeping-staff',
    title: 'Housekeeping Staff',
    category: 'Support & Logistics',
    popular: false,
    featured: false,
    employer: 'Facility Maintenance & Hygiene Services',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Udaipur', 'Jodhpur'],
    primaryLocation: 'Jaipur & Delhi NCR',
    salaryMin: 14000,
    salaryMax: 20000,
    salaryPeriod: 'month',
    salaryDisplay: '₹14,000 – ₹20,000 / month',
    experienceRequired: '0 - 2 years (Freshers Welcome)',
    experienceLevel: 'Fresher / Experienced',
    qualification: 'Below 10th / 10th Pass',
    shift: '8 Hours (Day Shift)',
    jobType: 'Full-time',
    vacancies: 50,
    genderEligibility: 'Male / Female',
    postedDate: 'Recently Posted',
    summary: 'Keep commercial offices, hospitals, hotels, and schools clean, sanitized, and well maintained.',
    overview: 'Immediate openings for energetic housekeeping personnel across hospitals, schools, and offices. Timely salary and PF/ESIC benefits.',
    responsibilities: [
      'Perform floor mopping, dusting, and vacuuming of office premises',
      'Sanitize washrooms and replenish soap, tissues, and sanitizers',
      'Clear trash bins and maintain waste segregation standards',
    ],
    requirements: ['Punctual, hardworking, and cooperative', 'Age: 18 - 48 years'],
    benefits: ['PF, ESIC, Uniform, Overtime bonus'],
    documentsRequired: ['Aadhaar Card', 'Bank Account Passbook'],
  },
  {
    id: 'sj-013',
    slug: 'cash-van-driver',
    title: 'Cash Van Driver',
    category: 'Support & Logistics',
    popular: false,
    featured: false,
    employer: 'SecureCash Logistics India',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Jodhpur', 'Udaipur', 'Mumbai'],
    primaryLocation: 'Jaipur & Delhi NCR',
    salaryMin: 20000,
    salaryMax: 28000,
    salaryPeriod: 'month',
    salaryDisplay: '₹20,000 – ₹28,000 / month',
    experienceRequired: '2+ years with Commercial Driving Licence (LMV/HMV)',
    experienceLevel: 'Experienced Commercial Driver',
    qualification: '10th Pass',
    shift: 'Day Shift (Route Based)',
    jobType: 'Full-time',
    vacancies: 15,
    genderEligibility: 'Male',
    postedDate: '4 days ago',
    summary: 'Drive armored cash transit vans safely between bank branches, currency vaults, and ATM kiosks.',
    overview: 'Drive specially armored cash transit vehicles with security armed guards and custodians along authorized bank replenishment routes.',
    responsibilities: [
      'Safely operate cash transit vans adhering to traffic and security speed protocols',
      'Perform pre-trip vehicle checks (brakes, tires, GPS tracker, communication radio)',
      'Follow designated secure route navigation without unauthorized stops',
    ],
    requirements: ['Valid Commercial Driving Licence (LMV-TR / HMV)', 'Clean driving record and police clearance'],
    benefits: ['PF, ESIC, Accident Insurance, Trip allowance'],
    documentsRequired: ['Commercial Driving Licence', 'Aadhaar & PAN Card', 'Address Proof'],
  },
  {
    id: 'sj-014',
    slug: 'atm-custodian',
    title: 'ATM Custodian',
    category: 'Support & Logistics',
    popular: false,
    featured: false,
    employer: 'Financial Services & Cash Replenishment Corp',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Jodhpur', 'Kota', 'Mumbai'],
    primaryLocation: 'Jaipur & Delhi NCR',
    salaryMin: 20000,
    salaryMax: 28000,
    salaryPeriod: 'month',
    salaryDisplay: '₹20,000 – ₹28,000 / month',
    experienceRequired: '1 - 3 years',
    experienceLevel: 'Experienced',
    qualification: '12th Pass / Graduate',
    shift: 'Day Shift (Route Schedule)',
    jobType: 'Full-time',
    vacancies: 12,
    genderEligibility: 'Male',
    postedDate: '5 days ago',
    summary: 'Responsible for loading cash cassettes into ATM machines, clearing paper jams, and balancing physical audit receipts.',
    overview: 'Maintain accurate accounting and custody of currency during ATM loading runs accompanied by armed escort teams.',
    responsibilities: [
      'Load currency cassettes into ATMs following strict dual-combination lock security',
      'Collect purged cash, clear transaction errors, and replace receipt paper rolls',
      'Generate audit balance printouts and report end-of-day reconciliation to vault officers',
    ],
    requirements: ['12th pass with basic numeracy and integrity check', 'Clean background verification'],
    benefits: ['PF, ESIC, Route incentive, Group term insurance'],
    documentsRequired: ['Aadhaar Card, PAN Card', '12th / College Marksheet', 'Police Verification'],
  },
  {
    id: 'sj-015',
    slug: 'fire-marshal',
    title: 'Fire Marshal',
    category: 'Technical & Control Room',
    popular: false,
    featured: false,
    employer: 'Industrial Safety & Fire Protection Group',
    locations: ['Jaipur', 'Delhi', 'Noida', 'Gurugram', 'Mumbai', 'Ahmedabad'],
    primaryLocation: 'Industrial Hubs & Delhi NCR',
    salaryMin: 22000,
    salaryMax: 34000,
    salaryPeriod: 'month',
    salaryDisplay: '₹22,000 – ₹34,000 / month',
    experienceRequired: '1 - 4 years with Fire Safety Diploma/Certificate',
    experienceLevel: 'Certified Safety Specialist',
    qualification: '10th/12th + Fire & Safety Certification',
    shift: 'Rotational 8/12 Hours',
    jobType: 'Full-time',
    vacancies: 10,
    genderEligibility: 'Male',
    postedDate: '1 week ago',
    summary: 'Conduct fire extinguisher inspections, inspect smoke alarms, lead evacuation drills, and enforce site fire safety standards.',
    overview: 'Specialized fire safety technician deployed at industrial plants, high-rise office towers, warehouses, and shopping complexes.',
    responsibilities: [
      'Inspect fire hydrant systems, water pressure gauges, smoke detectors, and fire extinguishers',
      'Conduct monthly mock fire evacuation drills for building tenants and staff',
      'Ensure emergency exit stairways and fire doors remain free of obstructions',
    ],
    requirements: ['Fire Safety Course Certificate (Sub-Officer / Fireman certification preferred)', 'Physically fit and proactive'],
    benefits: ['PF, ESIC, Safety risk allowance'],
    documentsRequired: ['Fire Safety Certification', 'Aadhaar Card', 'Educational Proof'],
  },
  {
    id: 'sj-016',
    slug: 'dog-handler',
    title: 'Dog Handler (K9)',
    category: 'Technical & Control Room',
    popular: false,
    featured: false,
    employer: 'Canine Security & Perimeter Patrol Services',
    locations: ['Delhi', 'Gurugram', 'Noida', 'Jaipur', 'Mumbai', 'Bengaluru'],
    primaryLocation: 'Delhi NCR & Airports/Warehouses',
    salaryMin: 22000,
    salaryMax: 32000,
    salaryPeriod: 'month',
    salaryDisplay: '₹22,000 – ₹32,000 / month',
    experienceRequired: '1+ years in K9 dog handling',
    experienceLevel: 'Specialized Canine Handler',
    qualification: '10th Pass',
    shift: 'Day / Night Shifts',
    jobType: 'Full-time',
    vacancies: 6,
    genderEligibility: 'Male',
    postedDate: '1 week ago',
    summary: 'Handle trained sniffer and guard dogs for perimeter patrolling, explosive detection, and warehouse perimeter security.',
    overview: 'Work with trained German Shepherds, Labradors, and Belgian Malinois for active perimeter patrolling and contraband detection.',
    responsibilities: [
      'Conduct perimeter walking rounds with trained guard/sniffer dogs',
      'Maintain dog feeding, grooming, and health check routines in coordination with agency vets',
      'Follow tactical command cues during public crowd events or warehouse patrols',
    ],
    requirements: ['Prior experience in dog handling or training', 'Genuine passion for animal welfare and safety'],
    benefits: ['PF, ESIC, Canine handling allowance'],
    documentsRequired: ['Aadhaar Card', 'Experience Certificate (if available)'],
  },
  {
    id: 'sj-017',
    slug: 'event-security-guard',
    title: 'Event Security Guard',
    category: 'Guard & Protection',
    popular: false,
    featured: false,
    employer: 'Exhibition & Concert Security Crew',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Goa', 'Udaipur'],
    primaryLocation: 'Jaipur, Delhi NCR & Udaipur',
    salaryMin: 18000,
    salaryMax: 26000,
    salaryPeriod: 'month',
    salaryDisplay: '₹18,000 – ₹26,000 / month (or Daily Pay ₹900 - ₹1400/shift)',
    experienceRequired: '0 - 2 years',
    experienceLevel: 'Fresher / Experienced',
    qualification: '10th Pass',
    shift: 'Event Shifts (Day/Night)',
    jobType: 'Full-time / Event Contract',
    vacancies: 40,
    genderEligibility: 'Male / Female',
    postedDate: 'Recently Posted',
    summary: 'Short-term and seasonal crowd management for destination weddings, trade expos, stadium concerts, and corporate summits.',
    overview: 'Great opportunity for candidates seeking high-energy event assignments in Rajasthan destination wedding palaces and Delhi NCR expo centers.',
    responsibilities: [
      'Manage venue ticket entry gates and wristband checkpoints',
      'Direct visitor vehicle parking in designated fairgrounds',
      'Assist guests with polite directions and maintain queue discipline',
    ],
    requirements: ['Neat grooming, polite behavior, and energetic attitude'],
    benefits: ['Meals provided during shifts', 'Prompt event payout options'],
    documentsRequired: ['Aadhaar Card', 'Bank Details'],
  },
  {
    id: 'sj-018',
    slug: 'security-inspector',
    title: 'Security Inspector',
    category: 'Supervision & Management',
    popular: false,
    featured: false,
    employer: 'National Security Quality Audit Group',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai'],
    primaryLocation: 'Delhi NCR & Jaipur',
    salaryMin: 28000,
    salaryMax: 40000,
    salaryPeriod: 'month',
    salaryDisplay: '₹28,000 – ₹40,000 / month',
    experienceRequired: '3 - 7 years in Security Compliance & Audits',
    experienceLevel: 'Senior Audit Specialist',
    qualification: 'Graduate / Ex-Army / Police',
    shift: 'General Shift + Field Visits',
    jobType: 'Full-time',
    vacancies: 8,
    genderEligibility: 'Male',
    postedDate: '6 days ago',
    summary: 'Perform surprise site compliance audits, weapon checks, guard biometric verification, and PSARA legal standard reviews.',
    overview: 'Ensure that all deployed security personnel adhere strictly to client service level agreements (SLAs), PSARA regulations, and agency standards.',
    responsibilities: [
      'Conduct unannounced day and night audit visits across client premises',
      'Score unit readiness: uniform, grooming, register completeness, weapon condition, and CCTV logs',
      'Submit detailed audit scoring sheets to branch operations directors',
    ],
    requirements: ['Strong analytical and report writing skills', 'Valid two-wheeler/four-wheeler driving licence'],
    benefits: ['PF, ESIC, Travel allowance, Performance bonuses'],
    documentsRequired: ['Aadhaar & PAN Card', 'Audit / Security Experience Proof', 'Degree'],
  },
  {
    id: 'sj-019',
    slug: 'security-manager',
    title: 'Security Manager',
    category: 'Supervision & Management',
    popular: false,
    featured: false,
    employer: 'Corporate Infrastructure Security Group',
    locations: ['Jaipur', 'Delhi', 'Gurugram', 'Noida', 'Mumbai', 'Bengaluru'],
    primaryLocation: 'Delhi NCR, Jaipur & Mumbai',
    salaryMin: 45000,
    salaryMax: 75000,
    salaryPeriod: 'month',
    salaryDisplay: '₹45,000 – ₹75,000 / month',
    experienceRequired: '6 - 15 years (Ex-Defence Officer / Corporate Security Head)',
    experienceLevel: 'Management Level',
    qualification: 'Graduate / Post Graduate / Ex-Army Captain/Major',
    shift: 'General Shift',
    jobType: 'Full-time',
    vacancies: 5,
    genderEligibility: 'Male / Female',
    postedDate: '1 week ago',
    summary: 'Lead entire campus security operations, direct disaster response planning, manage multi-agency vendors, and interface with senior management.',
    overview: 'Senior management leadership role accountable for physical security strategy, executive protection, asset risk mitigation, and compliance across large enterprise sites.',
    responsibilities: [
      'Formulate and execute campus-wide security protocols, SOPs, and emergency disaster recovery plans',
      'Manage contracts with security manpower agencies and electronic security system vendors',
      'Liaise directly with local police departments, fire authorities, and corporate risk directors',
      'Oversee internal investigations into loss, theft, data breaches, or safety violations',
    ],
    requirements: [
      'Ex-Commissioned Military Officer (Army/Navy/Air Force) or seasoned Corporate Security Manager with CPP/PSP certification preferred',
      'Exceptional leadership, crisis management, and stakeholder presentation capabilities',
    ],
    benefits: [
      'Executive salary package + annual performance bonus',
      'Health insurance for self and family',
      'Company vehicle / travel reimbursement',
    ],
    documentsRequired: [
      'Military Service Discharge Record (for Ex-Officers) / Corporate Relieving Letters',
      'Educational Degrees & Certifications',
      'Aadhaar & PAN Card',
    ],
  },
];

// Helper functions for filtering and search
export function getJobBySlug(slug) {
  if (!slug) return null;
  return JOBS_CATALOG.find((job) => job.slug.toLowerCase() === slug.toLowerCase()) || null;
}

export function filterJobs({
  role = '',
  category = '',
  city = '',
  experience = '',
  jobType = '',
  salaryMin = 0,
  gender = '',
  searchQuery = '',
}) {
  let filtered = [...JOBS_CATALOG];

  if (searchQuery && searchQuery.trim()) {
    const q = searchQuery.trim().toLowerCase();
    filtered = filtered.filter(
      (job) =>
        job.title.toLowerCase().includes(q) ||
        job.category.toLowerCase().includes(q) ||
        job.summary.toLowerCase().includes(q) ||
        job.locations.some((loc) => loc.toLowerCase().includes(q)) ||
        job.primaryLocation.toLowerCase().includes(q)
    );
  }

  if (role && role.trim()) {
    const r = role.trim().toLowerCase();
    filtered = filtered.filter((job) => job.slug.toLowerCase() === r || job.title.toLowerCase().includes(r));
  }

  if (category && category.trim() && category !== 'All') {
    filtered = filtered.filter((job) => job.category.toLowerCase() === category.toLowerCase());
  }

  if (city && city.trim() && city !== 'All') {
    const c = city.trim().toLowerCase();
    filtered = filtered.filter(
      (job) => job.locations.some((loc) => loc.toLowerCase() === c) || job.primaryLocation.toLowerCase().includes(c)
    );
  }

  if (experience && experience.trim() && experience !== 'All') {
    if (experience.toLowerCase().includes('fresher')) {
      filtered = filtered.filter((job) => job.experienceRequired.toLowerCase().includes('fresher') || job.experienceLevel.toLowerCase().includes('fresher'));
    } else if (experience.toLowerCase().includes('experienced')) {
      filtered = filtered.filter((job) => !job.experienceLevel.toLowerCase().startsWith('fresher only'));
    }
  }

  if (jobType && jobType.trim() && jobType !== 'All') {
    filtered = filtered.filter((job) => job.jobType.toLowerCase().includes(jobType.toLowerCase()));
  }

  if (salaryMin && Number(salaryMin) > 0) {
    filtered = filtered.filter((job) => job.salaryMax >= Number(salaryMin));
  }

  if (gender && gender.trim() && gender !== 'All') {
    if (gender.toLowerCase() === 'female') {
      filtered = filtered.filter((job) => job.genderEligibility.toLowerCase().includes('female'));
    } else if (gender.toLowerCase() === 'male') {
      filtered = filtered.filter((job) => job.genderEligibility.toLowerCase().includes('male'));
    }
  }

  return filtered;
}

export const POPULAR_JOB_ROLES = JOBS_CATALOG.filter((j) => j.popular);
export const FEATURED_JOB_ROLES = JOBS_CATALOG.filter((j) => j.featured);

export default JOBS_CATALOG;
